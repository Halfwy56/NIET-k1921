...
/* ---------------------------------------------------- кириллица 5x7 ------ */
/* Тот же колоночный формат. Индексы: 0..31 = А..Я, 32..63 = а..я, 64 = Ё/ё.
   Строчные буквы, кроме и/м/н/ц/ч, повторяют заглавные - на 5x7 они
   неотличимы, а для этих пяти рисуем строчное начертание (высотой в 5 строк). */
#define CYR_N  65
#define CYR_YO 64

static const uint8_t fontCyr[CYR_N][5] = {
    {0x7E,0x09,0x09,0x09,0x7E}, /* А */ {0x7F,0x49,0x49,0x49,0x31}, /* Б */
    {0x7F,0x49,0x49,0x49,0x36}, /* В */ {0x7F,0x01,0x01,0x01,0x01}, /* Г */
    {0x60,0x3E,0x21,0x3F,0x60}, /* Д */ {0x7F,0x49,0x49,0x49,0x41}, /* Е */
    {0x7F,0x08,0x7F,0x08,0x7F}, /* Ж */ {0x22,0x41,0x49,0x49,0x36}, /* З */
    {0x7F,0x10,0x08,0x04,0x7F}, /* И */ {0x7E,0x11,0x08,0x05,0x7E}, /* Й */
    {0x7F,0x08,0x14,0x22,0x41}, /* К */ {0x40,0x3E,0x01,0x01,0x7F}, /* Л */
    {0x7F,0x02,0x04,0x02,0x7F}, /* М */ {0x7F,0x08,0x08,0x08,0x7F}, /* Н */
    {0x3E,0x41,0x41,0x41,0x3E}, /* О */ {0x7F,0x01,0x01,0x01,0x7F}, /* П */
    {0x7F,0x09,0x09,0x09,0x06}, /* Р */ {0x3E,0x41,0x41,0x41,0x22}, /* С */
    {0x01,0x01,0x7F,0x01,0x01}, /* Т */ {0x27,0x48,0x48,0x48,0x3F}, /* У */
    {0x1C,0x22,0x7F,0x22,0x1C}, /* Ф */ {0x63,0x14,0x08,0x14,0x63}, /* Х */
    {0x3F,0x20,0x20,0x3F,0x60}, /* Ц */ {0x07,0x08,0x08,0x08,0x7F}, /* Ч */
    {0x7F,0x40,0x7F,0x40,0x7F}, /* Ш */ {0x3F,0x20,0x3F,0x20,0x7F}, /* Щ */
    {0x01,0x7F,0x48,0x48,0x30}, /* Ъ */ {0x7F,0x48,0x30,0x00,0x7F}, /* Ы */
    {0x7F,0x48,0x48,0x48,0x30}, /* Ь */ {0x22,0x41,0x49,0x49,0x3E}, /* Э */
    {0x7F,0x08,0x7F,0x41,0x3E}, /* Ю */ {0x46,0x29,0x19,0x09,0x7F}, /* Я */
    /* строчные а..я */
    {0x7E,0x09,0x09,0x09,0x7E}, /* а */ {0x7F,0x49,0x49,0x49,0x31}, /* б */
    {0x7F,0x49,0x49,0x49,0x36}, /* в */ {0x7F,0x01,0x01,0x01,0x01}, /* г */
    {0x60,0x3E,0x21,0x3F,0x60}, /* д */ {0x7F,0x49,0x49,0x49,0x41}, /* е */
    {0x7F,0x08,0x7F,0x08,0x7F}, /* ж */ {0x22,0x41,0x49,0x49,0x36}, /* з */
    {0x7C,0x20,0x10,0x08,0x7C}, /* и */ {0x7E,0x11,0x08,0x05,0x7E}, /* й */
    {0x7F,0x08,0x14,0x22,0x41}, /* к */ {0x40,0x3E,0x01,0x01,0x7F}, /* л */
    {0x7C,0x08,0x10,0x08,0x7C}, /* м */ {0x7C,0x10,0x10,0x10,0x7C}, /* н */
    {0x3E,0x41,0x41,0x41,0x3E}, /* о */ {0x7F,0x01,0x01,0x01,0x7F}, /* п */
    {0x7F,0x09,0x09,0x09,0x06}, /* р */ {0x3E,0x41,0x41,0x41,0x22}, /* с */
    {0x01,0x01,0x7F,0x01,0x01}, /* т */ {0x27,0x48,0x48,0x48,0x3F}, /* у */
    {0x1C,0x22,0x7F,0x22,0x1C}, /* ф */ {0x63,0x14,0x08,0x14,0x63}, /* х */
    {0x3C,0x20,0x20,0x3C,0x60}, /* ц */ {0x0C,0x10,0x10,0x10,0x7C}, /* ч */
    {0x7F,0x40,0x7F,0x40,0x7F}, /* ш */ {0x3F,0x20,0x3F,0x20,0x7F}, /* щ */
    {0x01,0x7F,0x48,0x48,0x30}, /* ъ */ {0x7F,0x48,0x30,0x00,0x7F}, /* ы */
    {0x7F,0x48,0x48,0x48,0x30}, /* ь */ {0x22,0x41,0x49,0x49,0x3E}, /* э */
    {0x7F,0x08,0x7F,0x41,0x3E}, /* ю */ {0x46,0x29,0x19,0x09,0x7F}, /* я */
    {0x7F,0x4A,0x4B,0x4A,0x43}, /* Ё */
};
...
/* Строки в исходниках - UTF-8, кириллица занимает по 2 байта:
   D0 90..BF = А..п, D1 80..8F = р..я, D0 81 = Ё, D1 91 = ё. */
static const uint8_t *glyph_next(const unsigned char **pp)
{
    unsigned c = *(*pp)++;
    ...
    if (c == 0xD0u || c == 0xD1u) {
        ...
        if (c2) {
            (*pp)++;
            if      (c == 0xD0u && c2 >= 0x90u && c2 <= 0xBFu) idx = (int)(c2 - 0x90u);
            else if (c == 0xD1u && c2 >= 0x80u && c2 <= 0x8Fu) idx = 48 + (int)(c2 - 0x80u);
            else if (c == 0xD0u && c2 == 0x81u) idx = CYR_YO;
            else if (c == 0xD1u && c2 == 0x91u) idx = CYR_YO;
        }
        if (idx >= 0) return fontCyr[idx];
        return font5x7['?' - 0x20];
    }

    if (c >= 'a' && c <= 'z') c -= 32;          /* строчная латиница -> заглавная */
    if (c < 0x20 || c > 0x7F) c = '?';
